...
const PlayVideo = () => {
    ...
    const fetchOtherData = async () => {
        const videoChannel_url = ` https://youtube.googleapis.com/youtube/v3/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${apiData.snippet.channelId}&key=${API_KEY}`;
        await fetch(videoChannel_url).then(res =>res.json()).then(data => setChannelData(data.items[0]));
  ...
  return (
    <div className='play-video'>
      ...
      <div>
        <span><img src={like} alt="" />{apiData?value_converter(apiData.statistics.likeCount):176}</span>
      ...
      <div className="vid-description">
        <p>{apiData?apiData.snippet.description.slice(0,454): "Description Here"}</p>
        <hr />
